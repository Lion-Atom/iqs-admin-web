    ...
    <el-row :gutter="20">
      ...
      <el-col :xs="15" :sm="18" :md="19" :lg="20" :xl="20">
        ...
        >
          ...
          <el-table-column v-if="user.isAdmin" label="重置密码" align="center">
            <template slot-scope="scope">
              <el-button size="mini" type="text" @click="initPass(scope.row)">重置密码</el-button>
            </template>
          </el-table-column>
...
<script>
import crudUser, {editUser, initPass} from '@/api/system/user'
  ...
  methods: {
    ...
    [CRUD.HOOK.afterToCU](crud, form) {
      ...
      if (form.id == null) {
        ...
        this.getSupJobs(form.jobs[0].id)
      }
      this.getRoleLevel()
      // this.getJobs()
    ...
    loadJobs({action, parentNode, callback}) {
      if (action === LOAD_CHILDREN_OPTIONS) {
        getJobs({enabled: true, pid: parentNode.id}).then(res => {
          parentNode.children = res.content.map(function (obj) {
            if (obj.hasChildren) {
              obj.children = null
            }
