              ...
              <div>
                ...
                >
                  <el-form ref="taskForm" :model="taskForm" size="small" :label-width="formLabelWidth">
                    ...
                    <el-row v-if="taskForm.type !== '8D'">
                      ...
                      <el-col v-if="taskForm.type === '审核人员批准'" :span="12">
                        <el-form-item label="认证时间">
                          <el-input v-model="taskForm.srcPath" disabled />
                        </el-form-item>
                      </el-col>
                    </el-row>
                    <el-row v-if="taskForm.type !== '8D'">
                      ...
                      <el-col v-if="taskForm.type === '审核人员批准'" :span="12">
                        <el-form-item label="审核单位">
                          <el-input v-model="taskForm.suffix" disabled />
                        </el-form-item>
                      </el-col>
                    </el-row>
                    <el-row v-if="taskForm.type !== '8D'">
                    ...
                    <!--审核人员批准对应的附件列表-->
                    <el-row v-if="taskForm.type === '审核人员批准'">
                      <div v-for="(item,index) in auditorFiles" :key="item" class="text item">
                        <el-row>
                          <el-col :span="23">
                    ...
                    <!--审核计划批准对应的附件列表-->
                    <el-row v-if="taskForm.type === '审核计划'">
                      <div v-for="(item,index) in planFiles" :key="item" class="text item">
                        <el-row>
                          <el-col :span="23">
                            <el-form-item :label="'计划附件'+ (index+1) ">
                              <el-input v-model="item.path" disabled />
                            </el-form-item>
                          </el-col>
                          <el-col :span="1">
                    ...
                    <el-row v-if="taskForm.type === '审核计划'">
                      <div v-for="(item,index) in planTemplateFiles" :key="item" class="text item">
                        <el-row>
                          <el-col :span="23">
                            <el-form-item :label="'模板附件'+ (index+1) ">
                              <el-input v-model="item.path" disabled />
                            </el-form-item>
                          </el-col>
                          <el-col :span="1">
                    ...
                    <el-row v-if="taskForm.type !== '8D' && taskForm.approveResult === false ">
                      <el-col :span="24">
                    ...
                    >
                      <el-collapse v-model="activeNames" accordion>
                        <el-collapse-item title="问题明细" name="1" class="collapse-item">
                          ...
                          <el-row>
                            <el-col :span="12">
                              <el-form-item label="审批选择" required>
                                <el-select
                                  v-model="taskForm.approveResult"
                                  size="small"
                                  placeholder="审批结果"
                          ...
                          <el-row v-if="taskForm.approveResult === false">
                            <el-col :span="24">
                              <el-form-item label="驳回理由" required>
                                <el-input
...
import { getAuditorFileByAuditorId } from '@/api/tools/auditorFile'
...
export default {
  ...
  methods: {
    ...
    getMsgFormSon(msg) {
      ...
      } else if (msg.data.type === '审核人员批准') {
        this.getAuditorFileById(msg.data.storageId)
      } else if (msg.data.type === '审核计划') {
        // alert("审核计划标识："+msg.data.storageId)
        this.getPlanFileById(msg.data.storageId)
        this.getPlanByPlanId(msg.data.storageId)
      }
    ...
    // 查询审核人员信息
    getAuditorFileById(id) {
      this.auditorFiles = []
      getAuditorFileByAuditorId(id).then(res => {
        // alert(JSON.stringify(res))
        this.auditorFiles = res
      })
    },
    // 查询审核计划基础信息及其自定义模板附件信息
    getPlanByPlanId(id) {
      // alert(id)
      getAuditPlanById(id).then(res => {
        // alert(JSON.stringify(res))
        this.templateId = res.templateId
        if (validIsNotNull(this.templateId)) {
          this.planTemplateFiles = []
          getAuditPlanFileByCond(id, this.templateId).then(res => {
            this.planTemplateFiles = res
            // alert(JSON.stringify(this.planFiles))
          })
        }
      })
    },
    ...
    getPlanFileById(id) {
      // alert(id)
      this.planFiles = []
      getAuditPlanFileByCond(id, 0).then(res => {
        this.planFiles = res
        // alert(JSON.stringify(this.planFiles))
      })
    },
    ...
    submitIssue() {
      ...
      if (this.taskForm.approveResult === false) {
        this.issueForm.status = '驳回'
      }
    ...
    // 双击选中的行列，进入审批主页
    dbSelected(row) {
      // alert(JSON.stringify(row.bindingId))
      this.checkboxT(row)
      if (row.type === '文件') {
        let storageId = row.storageId
        if(row.changeType === 'REVISE 修改' && validIsNotNull(row.bindingId)) {
          // 若是文件修改则需要找到真实的文件标识，并传入跳转参数中去
          storageId = row.bindingId
        }
        this.$router.push(
          {
      ...
      } else if (row.type === '审核人员批准') {
        this.$router.push(
          {
            path: '/audit/auditor',
            query: {}
          })
      } else if (row.type === '审核计划') {
        this.$router.push(
          {
            path: '/audit/plan',
            query: {}
