  ...
  <div @dblclick="back" id="pdfDom" class="app-container">
    ...
    <el-collapse v-if="type==='系统8D'" v-model="activeNames" @change="handleChange">
      <el-collapse-item  class="collapse-title" title="D1 - 问题介绍和小组定义" name="1">
        <FirstForm class="item" :issue-id="issueId" :need-confirm="confirmVisible"/>
      </el-collapse-item>
      <el-collapse-item class="collapse-title" title="D2 - 问题描述" name="2">
        <SecondForm class="item" :issue-id="issueId" :need-confirm="confirmVisible"/>
      </el-collapse-item>
      <el-collapse-item class="collapse-title" title="D3 - 短期围堵措施" name="3">
        <ThirdForm class="item" :issue-id="issueId" :need-confirm="confirmVisible"/>
      </el-collapse-item>
      <el-collapse-item class="collapse-title" title="D4 - 根本原因分析与验证" name="4">
        <ForthForm class="item" :issue-id="issueId" :init-fish="initFishData" :need-confirm="confirmVisible"/>
      </el-collapse-item>
      <el-collapse-item v-if="!isSpecial" class="collapse-title" title="D5 - 改善措施" name="5">
        <FifthForm class="item" :issue-id="issueId" :need-confirm="confirmVisible"/>
      </el-collapse-item>
      <el-collapse-item v-if="!isSpecial" class="collapse-title" title="D6 - 改善措施的有效性评估" name="6">
        <SixthForm class="item" :issue-id="issueId" :need-confirm="confirmVisible"/>
      </el-collapse-item>
